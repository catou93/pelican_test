...
void rsimLoadSolverOpts(SimStruct* S)
{
    ...
    */
    ...
    /* AutoSolverWithStiffness feature value */
    {
        const char* opt = "AutoSolverWithStiffness";
        autoSolverWithStiffness = 0;
        if ( (sa=mxGetField(pa,idx,opt)) != NULL ) {
            if ( !mxIsIntVectorWrapper(sa) || mxGetNumberOfElements(sa) != 1 ) {
                result = "error reading auto solver feature value";
                goto EXIT_POINT;
            }            
            autoSolverWithStiffness = (int_T) mxGetPr(sa)[0];
        } 
    }
