...
extern int_T autoSolverWithStiffness;
...
jmp_buf gblRapidAccelSFcnJmpBuf;
...
bool rsimShouldStiffBeCalcForRunTimeSlvrSwitch( SimStruct *S ) 
{
    ...
    static const unsigned int SL_CS_STATUS_AUTO_ZCE = 0x800U;
    static const unsigned int SL_CS_STATUS_AUTO_TIME = 0x1000U;

    bool autoSolverRunTimeSwitching = ( ( autoSolverStatusFlags & SL_CS_STATUS_RUNTIME_AUTO ) > 0);
    if( !autoSolverWithStiffness && !autoSolverRunTimeSwitching ) return false;
    ...
    if( autoSolverRunTimeSwitching ) { 
        ...
        if( ssGetSolverFoundContZcEvents(S) && 
              ( ( autoSolverStatusFlags & SL_CS_STATUS_AUTO_ZCE )>0) ) {
            return true;
        }
...
#pragma warning(pop)
    ...
    /* Error handling for unsupported s-function behaviours (calls to mex API etc.) */
    {
        int setJmpReturnValue =
            setjmp(gblRapidAccelSFcnJmpBuf);
